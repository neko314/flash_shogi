...
このビルド済みフォントはリポジトリに同梱されているので、追加のビルド作業なしで
そのまま使えます。未導入の環境では文字化け（豆腐や空白）として表示されるため、
その場合は `--theme=text` を使ってください。

同梱フォントがうまく使えない場合（壊れている、信用できない等）は、
手元で同じフォントをビルドし直すこともできます。手順は
[`font/README.md`](font/README.md) の「ビルドパイプライン」を参照してください。
