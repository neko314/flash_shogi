...
- **絵柄の方向性**: いらすとや(irasutofree.com)の「将棋駒34点セット」
  イラスト(著作権の都合で素材そのものは使わない、雰囲気のみ参考)を見て、
  クリーム〜薄茶色の暖色・丸みのあるタッチに寄せる方向に変更。
  `preview.html` の「イラスト調(色付き・丸み)試作」セクションで、角を
  丸めた五角形+暖色の塗り+丸ゴシック(Zen Maru Gothic)の試作を比較中
  - 仕組み(PUAフォント、コードポイント割り当て)は変更なし。見た目だけの方針転換
  - 色付きの塗りを実フォントに反映するには、これも COLR/CPAL カラーフォント
    形式が前提になる(上記の色分け方針と合流する)
...
- 成り駒を赤字にする案(P案)も比較中。**枠(五角形)は黒のまま、文字だけ赤に
  する**方向で確定。
  - 通常のOpenType/TrueTypeフォントは1グリフ=単色インクのため、「枠は黒・
    文字だけ赤」を1文字の中で両立するには **COLR/CPALカラーフォント**
    (レイヤーごとに色を持てる形式。FontForgeで作成可能だが手順は増える)が
    必要になる見込み。アプリの Renderer 側で ANSI 色を変える方式は、1文字
    全体が同じ色になってしまう(枠だけ黒に残せない)ため、この要件には使えない
  - FontForge工程で、成香・成桂・成銀(と将来的に馬・龍なども検討)を
    COLR/CPAL対応グリフとして作る前提で進める
